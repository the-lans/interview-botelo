...
- Регистрация не должна оставлять пользователя в БД, если письмо верификации не удалось отправить: после временной SMTP-ошибки запрос должен оставаться безопасно повторяемым.
- CSRF‑защита.
- Rate‑limit на логин.
